    ...
    return Library((b) => b
      ...
      ..body.add(Enum((b) => b
        ...
        ..methods.add(Method((b) => b
          ...
          ..returns = rRef
          ..lambda = true
          ..body = rRef.property('circular').call([refer('radius')]).code))
        ..methods.add(Method((b) => b
          ..name = 'all'
          ..type = MethodType.getter
          ..returns = brRef
        ...
        ..methods.add(Method((b) => b
          ...
          ..body = brRef.property('only').call([], {
            'topLeft': refer('topLeft')
                .conditional(refer('asRadius'), rRef.property('zero')),
            'topRight': refer('topRight')
                .conditional(refer('asRadius'), rRef.property('zero')),
            'bottomLeft': refer('bottomLeft')
                .conditional(refer('asRadius'), rRef.property('zero')),
            'bottomRight': refer('bottomRight')
                .conditional(refer('asRadius'), rRef.property('zero')),
          }).code))
        ..methods.add(Method((b) => b
          ..name = 'combine'
          ...
          ..body = refer('all').property('copyWith').call([], {
            ...
            'topRight': refer('topRight').nullSafeProperty('asRadius'),
            'bottomLeft': refer('bottomLeft').nullSafeProperty('asRadius'),
            'bottomRight': refer('bottomRight').nullSafeProperty('asRadius'),
          }).code))
        ..values.addAll(config.radii.map((r) => EnumValue((b) => b
          ..name = r.validName
          ..docs.addAll(['/// ${r.validName} (${r.radius}px)'])
          ..arguments.add(refer('AppBorderRadii').property(r.validName))))))));
  }
}
